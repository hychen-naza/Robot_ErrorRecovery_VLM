...
ACTION_NAME_TO_MOVEMENT={
    ...
    "move left": -rotate_degree,
    "move right": rotate_degree,
    "stop": [0, 0]
}
...
def detect_grasp_object(current_image):
    ...
    if len(contours) == 0:
        return current_image, frame_threshed, clean, None, 0
...
def run_callee(prompt_configuration,loggin_dict, log_save_dir, connection_kwargs, max_steps=10, num_seeds=1, fake_marker_type=None, rows=1, cols=1, rotation_type=None, width=640, height=480):
    ...
    def save_logs(seed, step, log_save_dir, hor_raw_msg, summarized_hor_action, combined_action, current_image, coverage, dists):
        # logging 
    ...
    dists_all = []
    # for _ in range(5):
    #     pos_rotate = random.randint(25, 35)
    #     neg_rotate = random.randint(-25, -15)
    for seed in range(10):
        ...
        pos_rotate = random.randint(25, 35)
        neg_rotate = random.randint(-25, -15)
        rotate_degree = pos_rotate if seed % 2 == 0 else neg_rotate
        dists.append(rotate_degree)
        cmd += str(rotate_degree)
        parent_conn.send(cmd)  # Send command through the pipe
        response = parent_conn.recv()  # Wait for the response
        print(f"Initial movement success: {response}")
        ...
        for step in range(max_steps):
            ...
            cv2.rectangle(current_image, (after_box.cx-5, after_box.cy-15), (after_box.cx+5, after_box.cy+15), (0, 0, 255), -1) # fill 
            ...
            if hor_action == [0, 0]: # neither left nor right, means we arrived at the target
                step = max_steps-1
                hor_action = 0
            combined_action = hor_action * step_decay
            dists.append(dists[-1]+combined_action)
            print(f"Combined action: {combined_action}")
            coverage = 0
            save_logs(seed, step, log_save_dir, hor_raw_msg, summarized_hor_action, combined_action, current_image, coverage, dists[-1]+combined_action)
            ...
            if step == max_steps-1:
                ...
                save_logs(seed, step+1, log_save_dir, hor_raw_msg, summarized_hor_action, combined_action, after_image, coverage, None)
                ...
                # np.save('dists.npy', np.array(dists_all), allow_pickle=True)
...
if __name__ == "__main__": 
    ...
    parser.add_argument("-m","--max_steps",type=int,default=15) 
